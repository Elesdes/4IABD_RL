...
import time
import pygame
from algorithms.monte_carlo_methods import (monte_carlo_es_on_tic_tac_toe_solo,
...
def play(env, policy):
    ...
    while running:
        ...
        if not env.is_game_over():
            choices = policy[str(env.board)]
            if isinstance(choices, dict):
                choices = max(choices, key=choices.get)
            env.act_with_action_id(choices)
            time.sleep(0.5)
            ...
            if event.type == pygame.QUIT:
                ...
                if (
                    ...
                    if (
                        ...
                        policy = monte_carlo_es_on_tic_tac_toe_solo(env).pi
                        play(env, policy)
                    elif (
                        WINDOW_HEIGHT / 2 - BUTTON_HEIGHT - BUTTON_SPACING * 0.5
                        < mouse_pos[1]
                        ...
                        policy = on_policy_first_visit_monte_carlo_control_on_tic_tac_toe_solo(env, num_episodes=20000).pi
                        play(env, policy)
                    elif (
                        WINDOW_HEIGHT / 2 + BUTTON_SPACING * 0.5
                        < mouse_pos[1]
                        ...
                        policy = off_policy_monte_carlo_control_on_tic_tac_toe_solo(env).pi
                        play(env, policy)
                    elif (
                        WINDOW_HEIGHT / 2 + BUTTON_HEIGHT + BUTTON_SPACING * 1.5
                        < mouse_pos[1]
