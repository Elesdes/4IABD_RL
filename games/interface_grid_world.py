...
def draw_grid():
    for y, x in itertools.product(range(column_len), range(row_len)):
...
def draw_game_over():
    ...
    )  # Remplacez 'Game Over' par le message que vous voulez.
...
def draw_move_count(move_count):
    ...
    window.blit(
        text, (20, 20)
    )  # Dessine le texte en haut à gauche. Modifiez les coordonnées si vous le souhaitez.
...
def play_game():
    ...
    while running:
        for event in pygame.event.get():
            ...
            ):  # Cliquez sur le bouton X de la fenêtre pour quitter.
                running = False
            elif event.type == pygame.KEYDOWN:
                prev_state_id = env.state_id()
                print(env.available_actions())
                if (
                    event.key == pygame.K_LEFT and 0 in env.available_actions()
                ):  # Gauche
                    env.step(0)
                    move_count += 1
                elif (
                    event.key == pygame.K_RIGHT and 1 in env.available_actions()
                ):  # Droite
                    env.step(1)
                    move_count += 1
...
def play():
    ...
    while running:
        ...
        if not env.is_game_over():
            ...
            move_count += 2 if env.state_id() == prev_state_id else 1
            time.sleep(0.5)
            _extracted_from_play_29(env, move_count)
        else:
            _extracted_from_play_29(env, move_count)
            time.sleep(0.5)
...
def _extracted_from_play_29(env, move_count):
    draw_grid()
    draw_agent(env.state_id())
    draw_move_count(move_count)
