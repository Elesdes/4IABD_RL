            ...
            for a in env.actions():
                ...
                for s_p in range(len(env.states())):
                    for r in range(len(env.rewards())):
                        total_inter += env.transition_probability(s, a, s_p, r) * (env.rewards()[r] + 0.99999 * V[s_p])
    ...
    while True:
        ...
        while True:
            ...
            for s in range(len(env.states())):
                ...
                for s_p in range(len(env.states())):
                    for r in range(len(env.rewards())):
                        total += env.transition_probability(s, pi[s], s_p, r) * (env.rewards()[r] + 0.99999999 * V[s_p])
                V[s] = total
                delta = max(delta, np.abs(old_v - V[s]))
            if delta < theta:
                break
        ...
        for s in range(len(env.states())):
            ...
            for a in env.actions():
                ...
                for s_p in range(len(env.states())):
                    for r in range(len(env.rewards())):
                        total += env.transition_probability(s, a, s_p, r) * (env.rewards()[r] + 0.999999 * V[s_p])
                if best_a is None or total > best_action_score:
                    best_a = a
                    best_action_score = total
    ...
    while True:
        ...
        for s in range(len(env.states())):
            ...
            for a in env.actions():
                ...
                for s_p in range(len(env.states())):
                    for r in range(len(env.rewards())):
                        total += env.transition_probability(s, a, s_p, r) * (env.rewards()[r] + 0.99999999 * V[s_p])
